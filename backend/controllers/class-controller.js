...
const sclassCreate = async (req, res) => {
    try {
        ...
        if (existingSclassByName) {
            res.send({ message: 'Essa turma ja existe' });
        }
        else {
...
const sclassList = async (req, res) => {
    try {
        ...
        if (sclasses.length > 0) {
            ...
            res.send({ message: "Turmas nao encontradas" });
        }
    } catch (err) {
        res.status(500).json(err);
    }
...
const getSclassDetail = async (req, res) => {
    try {
        ...
        if (sclass) {
            ...
            res.send({ message: "Turma nao encontrada" });
        }
    } catch (err) {
        res.status(500).json(err);
    }
...
const deleteSclass = async (req, res) => {
    try {
        ...
        if (!deletedClass) {
            return res.send({ message: "Turma nao encontrada" });
        }
...
const deleteSclasses = async (req, res) => {
    try {
        ...
        if (deletedClasses.deletedCount === 0) {
            return res.send({ message: "Turmas nao encontradas" });
        }
